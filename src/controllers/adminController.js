...
const registerStudent = async (req, res) => {
    ...
    try {
        ...
        await logAdminAction(req.admin.id, 'STUDENT_REGISTERED', `Admin ${admin.name} registered student: ${firstName} ${surname} (${email}), RegNo: ${registrationNumber}`, 'student', result.lastID, req.ip);
        ...
        if (computerCourse) {
            ...
            await logAdminAction(req.admin.id, 'STUDENT_AUTO_ENROLLED', `Student ${firstName} ${surname} (ID: ${studentId}) auto-enrolled in ${courseNameToEnroll} (ID: ${computerCourse.id})`, 'enrollment', null, req.ip);
...
const saveComputerClassMarks = async (req, res) => {
    ...
    try {
        ...
        await logAdminAction(req.admin.id, 'STUDENT_MARKS_UPDATED', `Admin ${req.admin.name} updated marks for enrollment ID ${enrollmentId}.`, 'enrollment', enrollmentId, req.ip);
...
const finishCourseForStudent = async (req, res) => {
    ...
    try {
        ...
        await logAdminAction(req.admin.id, 'STUDENT_COURSE_FINISHED', `Admin ${req.admin.name} marked course as finished for enrollment ID ${enrollmentId}. Status: ${finalGradeToSet}`, 'enrollment', enrollmentId, req.ip);
    ...
    try {
        await sendEmailWithTemplate({
            ...
            data: {
                subjectLine: subject,
                htmlBody: emailBody,
                siteUrl: process.env.FRONTEND_URL || 'https://twoemcyberkagwe.onrender.com'
            },
...
const createDocument = [
    ...
    async (req, res) => {
        ...
        try {
            ...
            await logAdminAction(req.admin.id, 'DOCUMENT_CREATED', `Admin ${req.admin.name} added document: ${title}`, 'downloadable_document', result.lastID, req.ip);
...
const updateDocument = [
    ...
    async (req, res) => {
        ...
        try {
            ...
            else { await logAdminAction(req.admin.id, 'DOCUMENT_UPDATED', `Admin ${req.admin.name} updated document: ${title}`, 'downloadable_document', docId, req.ip); req.flash('success_msg', 'Document entry updated successfully.'); }
...
const deleteDocument = async (req, res) => {
    ...
    try {
        ...
        else { await logAdminAction(req.admin.id, 'DOCUMENT_DELETED', `Admin ${req.admin.name} deleted document entry ID: ${docId}`, 'downloadable_document', docId, req.ip); req.flash('success_msg', 'Document entry deleted successfully.');}
        res.redirect('/admin/documents');
    } catch (err) {
...
const adminResetStudentPassword = async (req, res) => {
    ...
    try {
        ...
        await logAdminAction( adminPerformingAction.id, 'STUDENT_PASSWORD_RESET_BY_ADMIN', `Admin ${adminPerformingAction.name} reset password for student ${student.first_name} (ID: ${studentId}) to default.`, 'student', studentId, req.ip );
