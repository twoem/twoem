...
exports.markNotificationAsRead = async (req, res) => {
    try {
        const notification = await Notification.findByIdAndUpdate(
            req.params.id,
            { $addToSet: { readBy: req.session.studentId } },
            { new: true }
        );
...
// WiFi Information
exports.showWifiInfo = (req, res) => {
    res.render('student/wifi-info', {
        title: 'Twoem | WiFi Information',
        wifiInfo: {
            ssid: 'TWOEM_STUDENT',
            password: 'StudentPortal2023',
            instructions: '1. Connect to TWOEM_STUDENT network\n2. Enter the password when prompted\n3. Accept terms if shown',
            disclaimer: 'Do not share this password with unauthorized users'
        }
    });
};
...
exports.submitSupportRequest = async (req, res) => {
    try {
        ...
        await sendEmail({
            ...
            subject: `Support Request from ${student.regNumber}: ${title}`,
            text: `From: ${student.fullName} (${student.regNumber})\n\n${message}\n\nReply to: ${student.email}`,
            html: `<p><strong>From:</strong> ${student.fullName} (${student.regNumber})</p>
                   <p><strong>Message:</strong></p>
                   <p>${message.replace(/\n/g, '<br>')}</p>
                   <p><strong>Reply to:</strong> <a href="mailto:${student.email}">${student.email}</a></p>`
        });
        
        res.render('student/support', {
            title: 'Twoem | Support',
            success: 'Your support request has been submitted. We will respond via email.'
        });
    } catch (error) {
